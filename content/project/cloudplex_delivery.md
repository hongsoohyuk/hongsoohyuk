...
> 📖 **_개발 배경_**
>
> - CPD 프로젝트 관리용 웹사이트(CPD-Admin)가 필요했다.
> - 비즈니스 로직을 포함한 UI의 공통 모듈화 요구가 있었다.
> - 기존 CPD-Project에 TypeScript를 도입하고 API 데이터 모델을 통합할 필요가 있었다.
...
> 📢 **_개발 성과_**
>
> - cpd-admin 앱을 초기 설계부터 단독으로 구축하고, 조직(Space) 단위로 CDN 전체 리소스를 관리하는 대시보드를 완성.

### CDN 실시간 모니터링 시스템 (Pub/Sub 패턴)

> 📖 **_개발 배경_**
>
> - 스포츠 중계 등 장시간 스트리밍 이벤트 대응을 위한 실시간 CDN 상태 모니터링 및 라우팅 정책 즉시 수정 기능이 필요했다.
> - CDN 제공사가 실시간 데이터를 즉시 제공하지 않으므로 polling 방식의 데이터 수집이 필요했다.
> - 최대 3시간 운영 시 브라우저 부하를 최소화하기 위한 효율적 렌더링 전략이 필요했다.
...
> 📢 **_개발 성과_**
>
> - Pub/Sub 패턴 기반 데이터 변경 이벤트 관리로 구독 중인 셀만 재렌더링. 장시간 모니터링에서도 **브라우저 부하를 최소화하고 안정적인 성능을 확보**. (2024.04 ~ 2024.05)

### 라우팅 정책 V2 및 스케줄링 시스템

> 📖 **_개발 배경_**
> DNS 라우팅 정책의 차세대 버전이 요구되었다. 기존 대비 정책 유형 확장 및 시간 기반 자동 가중치 변경 기능이 필요했다.
...
> 📖 **_개발 배경_**
> 복잡한 DNS Zonefile 데이터를 직관적으로 시각화할 필요가 있었다.
...
> 📖 **_개발 배경_**
>
> - CDN별 일간 트래픽 데이터를 시각화하고 임계치 초과 시 자동 알림이 필요했다.
> - CDN 벤더별로 수집 가능한 지표가 다르므로 유연한 Preset 설정이 필요했다.

> 🔧 **_수행 내용_**
>
> - **일간 지표 차트** — Recharts 기반 누적 데이터 시각화 (Bandwidth/Request/Hit Ratio)
...
> 📢 **_개발 성과_**
>
> - **Slack Webhook과 Email 수신자를 연동한 자동 알림 시스템** 구축 완료.
...
> 📖 **_개발 배경_**
>
> - CPD-Admin 개발과 함께 디자인 시스템 및 비즈니스 로직을 포함한 UI의 공통 모듈화가 필요했다.
> - 기존 CPD-Project에 TypeScript를 도입하고 Admin 공용 API 데이터 모델을 통합할 필요가 있었다.
...
> 📢 **_개발 성과_**
>
> - Yarn-Berry PnP를 통해 **유령 의존성 발견 및 잠재적 이슈 해결**
> - 공통 UI 컴포넌트 공유화로 유지보수성과 확장성 향상
...
> 📖 **_개발 배경_**
>
> - AWS 개발 환경 통합 과정에서 기존 리소스를 동일 사양으로 재구성할 필요가 있었다.
> - CPD-Admin 개발과 동시에 배포 환경을 빠르게 재현하기 위한 자동화 인프라 요구가 있었다.
...
> 📖 **_개발 배경_**
>
> - 서버 작업(Job) 완료/실패 이벤트를 실시간으로 UI에 반영할 필요가 있었다.
> - 기존 polling 방식의 비효율적인 데이터 갱신 문제.

> 🔧 **_수행 내용_**
>
> - 토큰 인증 기반 SSE 연결, 채널 타입별(User/Project Job) 분리
> - Observer/Subscriber 패턴으로 항목 단위 업데이트 구현
