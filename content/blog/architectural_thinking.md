...
변경사항이 아키텍처의 확장성에 미칠 영향을 고려해야 한다.
...
### Design
바닥이 카펫인지, 원목 바닥인지, 벽지와 조명이 어떤지 → 설계와 관련된다.
UI의 느낌과 모양은 시스템의 설계를 정의한다.

---

## 아키텍처 vs 설계, 판단 기준
대부분의 결정은 아키텍처와 설계 사이의 스펙트럼 어딘가에 위치한다.
...
## 기술적 너비와 깊이
지식 피라미드 관점에서 아키텍트에게는 개발자와 다른 접근이 필요하다.
기술적 너비를 넓혀 본인이 아는 지식에만 매몰되지 않도록 경계해야 하고, 그 기술과 함께 사장되지 않도록 해야 한다.
...
비즈니스 도메인 지식과 이해관계자와의 원만하고 협력적인 관계가 필요하다.
...
대신 아키텍트는 소규모 비즈니스 기능성(예: UI 개발)을 담당하고, 시스템의 핵심은 개발팀에 일임하는 것이 좋은 방법일 수 있다.

### 개발 팀과 같은 코드를 작업할 수 없는 경우
1. **PoC를 자주 진행한다.** PoC에 필요한 소스는 아키텍트가 담당하며, 구현상의 세부사항과 전체 솔루션 개발에 필요한 비용의 양을 가늠할 수 있다.
2. **기술 부채 해결.** 기술 부채를 해결하는 작업은 한 Iteration 안에서 우선순위가 낮기 때문에, 완료하지 못하더라도 악영향이 없다.
3. **버그 수정.**
