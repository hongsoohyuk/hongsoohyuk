...
대규모 React 모노레포의 메인 앱(200+ 컴포넌트)을 플랫 디렉토리 구조에서 Feature-Sliced Design(FSD) 아키텍처로 점진적으로 전환한 경험을 정리했다. 약 50개의 커밋에 걸쳐 3개월간 진행한 마이그레이션의 전략, 도입 패턴, 그리고 실무에서 마주한 문제들을 다룬다.
...
- **합의를 문서화하자**
- **슬라이스 간 조합이 너무 많다면 FSD 도입 자체를 재고하자**: 도메인 간 경계가 명확한 서비스에서는 FSD가 잘 작동한다. 하지만 하나의 화면에서 여러 도메인의 데이터를 조합해야 하는 경우가 대부분이라면, cross-import와 상위 레이어 조합이 폭발적으로 늘어난다.
