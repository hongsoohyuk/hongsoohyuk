...
title: 'SaaSOps'
slug: 'saasops'
description: '기업용 SaaS 애플리케이션의 지출, 라이선스, 계약, 사용자, 워크플로우를 통합 관리하는 엔터프라이즈 어드민 플랫폼'
...
> 📖 **_개발 배경_**
>
> - SaaS 지출 등록을 위해 수동 입력, 카드 승인 내역, Invoice OCR 등 다양한 소스 지원이 필요했다.
...
> - **비용 대시보드** — 월별/연별 추이 차트, 벤더·조직·앱별 순위 차트 (8종), 차트 클릭 → 상세 패널 드릴다운
> - **환율 처리** — 워크스페이스/조직 기본 통화 기준 자동 환산, 원화/엔화 소수점 제거, 환율 적용일 기반 계산
> - **비용 소스 연동** — 계약 선택 시 사용 기간 자동 설정, OCR 결과 → 비용 자동 변환, 거래 내역 → 비용 생성
> - **비용 대상(Target)** — 앱 인스턴스/벤더 자동완성 선택 컴포넌트

> 📢 **_개발 성과_**
>
> - ViewModel 패턴 도입으로 **기존 대비 기능별 코드 30% 감소**, OCR 기반 자동완성과 수동 입력 폼의 결합을 간결하게 처리
...
> 📖 **_개발 배경_**
>
> - 인보이스 PDF를 수동으로 확인한 뒤 비용을 입력하는 과정을 간편화할 필요가 있었다.
> - OCR 분석 결과의 신뢰도(Confidence)를 반영하고, 사용자가 쉽게 수정할 수 있는 UX가 필요했다.

> 🔧 **_수행 내용_**
>
> - **OCR 작업 관리** — 목록, 상세 패널, 벌크 업로드, 분석 중 폴링 처리
> - **분석 결과 UI** — 인식값 호버/선택 시 필드 포커스, 신뢰도순 정렬
> - **OCR → Spend 변환** — 결과 기반 비용 생성 다이얼로그, 인보이스 파일 자동 첨부, 복수 비용 생성
> - **인보이스 메일 포워딩** — 매칭 룰 관리 (화이트/블랙리스트), 임계치 조정
> - **PDF/이미지 뷰어** — `@packages/ui`에 `react-pdf` 기반 뷰어 컴포넌트 추가

> 📢 **_개발 성과_**
>
> - 파일 업로드 → OCR 분석 → 결과 확인 → 비용 자동 변환까지의 **전체 파이프라인을 단독 구현**
> - 메일 포워딩 매칭 룰을 통한 **인보이스 자동 수집** 워크플로우 완성
...
> 📖 **_개발 배경_**
>
> - 기존에는 4개의 독립 프로젝트(admin, launcher, my-account)가 각각 별도 레포지토리로 관리되어 공통 로직 중복과 비효율적인 의존성 관리 문제가 있었다.
> - Webpack 기반 CRA(react-scripts) 환경에서 사용하지 않거나 유령 참조로 남은 의존성이 많아 개발 서버 구동 속도와 빌드 안정성 이슈가 발생했다.
...
> - 프론트엔드 챕터에서 Vite 전환 리뷰를 진행하며 지식 공유 문화에 기여
...
> 📖 **_개발 배경_**
>
> - admin 프로젝트가 비대해지면서 워크스페이스 도메인과 조직 도메인의 경계가 모호해지고, 코드 간 결합도가 높아지는 문제가 발생했다.
>   - 레거시 코드 중 `isWorkspace` `isAdmin` 등 boolean 타입 기반 props가 만연.
>   - Single responsibility principle 등 SOLID 원칙이 지켜지지 않음.
> - 기능 간 의존 관계 파악이 어렵고, 온보딩 비용이 높아졌다.
...
> 💡 **_배경_**
> Claude, Cursor 등 생성형 AI 도구를 활용하는 과정에서 남용 사례 발생.
>
...
> 🔧 AI 도구와 개발자 모두 참조할 수 있는 프로그래밍 컨벤션과 프로젝트 설계 문서(FSD) 도입
>
> | Problem                                                                                                                             | Solution                                                                                                                           |
> | ----------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
> | 하나의 파일 안에 Business Logic, UI Logic, 도메인별 조건 분기, UI type별 조건 분기, Magic number 등이 뒤섞여 최대 5,000라인에 달함. | 소프트웨어 설계 원칙(OOP 원칙, FSD 설계 원칙, Code Review Skills)을 기존 코드 리팩터링과 추가 기능 개발에 활용할 수 있도록 문서화. |
...
> 🔧 **_수행 내용_**
>
> - 커스텀 필드 CRUD 관리 페이지, 유형별 attribute 필드 구성 최적화
...
> - `@packages/ui`의 pinned column 로직을 공통화하고 Magic number 제거
...
> 📢 **_개발 성과_**
>
> - OpenAPI Generator로 5개 도메인(SMP, Organization, OCR, Application, File)의 interface/type을 **자동 생성**하는 파이프라인 구축
